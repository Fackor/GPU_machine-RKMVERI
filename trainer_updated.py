...
DICT_FILE = os.path.join(DATA_DIR, "category_map.txt")
...
TRAIN = bool(input("Set training on? [True|False]"))
if(TRAIN!=True):
...
#create the validation data layers
...
while(True):
    ...
    SINGLE_SENTENCE = bool(input("Classify a single sentence? "))
    if(SINGLE_SENTENCE!=True):
        break
    sentence = input("enter a sentence to classify ").lower()
    tmp_file = "tmp_sentence.tsv"
    with open(tmp_file, 'w+', encoding='utf-8') as tmp_tsv:
    ...
    tmp_probs = torch.nn.functional.softmax(torch.cat(tmp_logits_tensors[0]), dim=1).numpy()
    category = np.argmax(tmp_probs)

    print(f'{sentence} | {category_map[category]}')
